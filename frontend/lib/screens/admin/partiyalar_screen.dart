...
class _PartiyalarEkraniState extends State<PartiyalarEkrani> {
  ...
  Future<void> _yuklash() async {
    ...
    try {
      ...
      final javob = await context.read<AppState>().api.get('/partiyalar', query: query);
      setState(() => _sahifa = Sahifalangan.fromJson(javob, (e) => Partiya.fromJson(e)));
    } catch (e) {
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              TextField(controller: xaridorKontrolleri, decoration: InputDecoration(labelText: lok.t('xaridor'))),
              TextField(controller: sortKontrolleri, decoration: InputDecoration(labelText: lok.t('sort'))),
              TextField(
                  controller: uramaBilanKontrolleri,
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: InputDecoration(labelText: lok.t('urama_bilan_vazn'))),
              TextField(
              ...
              TextField(
                  ...
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: InputDecoration(labelText: lok.t('kondicion_vazni'))),
            ],
          ),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: Text(lok.t('bekor'))),
          FilledButton(
            onPressed: () async {
              try {
                await context.read<AppState>().api.post('/partiyalar/${p.id}/sotish', tana: {
                  'sotuv_sanasi': DateTime.now().toIso8601String().substring(0, 10),
                  'xaridor': xaridorKontrolleri.text.trim(),
                  'sort': sortKontrolleri.text.trim().isEmpty ? null : sortKontrolleri.text.trim(),
                  'urama_bilan_vazn': double.tryParse(uramaBilanKontrolleri.text) ?? 0,
                  'urama_vazni': double.tryParse(uramaKontrolleri.text) ?? 0,
                  'sof_vazn': double.tryParse(sofVaznKontrolleri.text) ?? 0,
                  'kondicion_vazni': double.tryParse(kondicionKontrolleri.text),
                });
      ...
      child: Column(
        ...
        children: [
          Wrap(
            spacing: 8,
            children: [
              ChoiceChip(label: Text(lok.t('ochiq')), selected: _holatiFiltri == 'ochiq', onSelected: (_) { setState(() => _holatiFiltri = 'ochiq'); _yuklash(); }),
              ChoiceChip(label: Text(lok.t('yopiq')), selected: _holatiFiltri == 'yopiq', onSelected: (_) { setState(() => _holatiFiltri = 'yopiq'); _yuklash(); }),
              ChoiceChip(label: Text(lok.t('sotildi')), selected: _holatiFiltri == 'sotilgan', onSelected: (_) { setState(() => _holatiFiltri = 'sotilgan'); _yuklash(); }),
              ActionChip(label: Text(lok.t('tozalash')), onPressed: () { setState(() => _holatiFiltri = null); _yuklash(); }),
            ],
          ),
          ...
          if (!_yuklanmoqda && _xato == null && _sahifa != null)
            Expanded(
              child: ListView(
                children: _sahifa!.items.map((p) => _partiyaKartasi(p, lok)).toList(),
              ),
            ),
        ],
      ),
  ...
  Widget _partiyaKartasi(Partiya p, dynamic lok) {
    final sotilganmi = p.holati == 'sotilgan';
    final rang = sotilganmi ? Colors.red : Colors.green;
    return Card(
      color: rang.withValues(alpha: 0.06),
      child: ListTile(
        leading: CircleAvatar(backgroundColor: rang, child: Text('#${p.partiyaRaqami}', style: const TextStyle(fontSize: 11, color: Colors.white))),
        title: Text('${p.mahsulotNomi} — Partiya #${p.partiyaRaqami}'),
        subtitle: Text('${p.kipSoni} ${lok.t("soni")} · ${p.jamiKg.toStringAsFixed(1)} kg · ${lok.t(p.holati == "ochiq" ? "ochiq" : p.holati == "yopiq" ? "yopiq" : "sotildi")}'
            '${p.xaridor != null ? " · ${p.xaridor}" : ""}${p.nakladnoyRaqami != null ? " · ${p.nakladnoyRaqami}" : ""}'),
        trailing: p.holati == 'ochiq'
            ? OutlinedButton(onPressed: () => _yopish(p), child: Text(lok.t('yopish')))
            : p.holati == 'yopiq'
                ? FilledButton(onPressed: () => _sotishFormasiniOchish(p), child: Text(lok.t('sotish')))
                : null,
      ),
    );
  }
